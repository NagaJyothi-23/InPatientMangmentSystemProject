...
export class DoctorService {
  ...
  private apiUrl = 'http://localhost:8103/patient-service/doctor';
  ...
  getPatients(name: String): Observable<any[]> {
    return this.http.get<any[]>(
      `http://localhost:8103/patient-service/patients/getByDoctor/${name}`
    );
  }

  getDetails(): Observable<any[]> {
    return this.http.get<any[]>(
      `http://localhost:8103/patient-service/doctor/getAllWithDept`
    );
  }
